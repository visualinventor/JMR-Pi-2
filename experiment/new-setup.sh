...
do_advanced_menu() {
  FUN=$(whiptail --title "JMRPi3 Configuration Tool" --menu "Advanced Options" $WT_HEIGHT $WT_WIDTH $WT_MENU_HEIGHT --cancel-button Back --ok-button Select \
...
calc_wt_size
while true; do
  FUN=$(whiptail --title "JMRPi3 Configuration Tool" --menu "Setup Options" $WT_HEIGHT $WT_WIDTH $WT_MENU_HEIGHT --cancel-button Finish --ok-button Select \
    "1 Setup new JMRI system" "This requires a new Raspberry pi OS install. It installs everything needed for wi-fi hotspot functionality and JMRI" \
